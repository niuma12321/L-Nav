...
// Services
import { fetchMarketIndices, fetchStockData, IndexData } from '../../services/financeService';
                ...
                <div className="grid grid-cols-3 gap-3 mt-4">
                  {[
                    ...
                    <div key={item.name} className="p-3 rounded-2xl bg-[#0d0e10]">
                      ...
                      <p className={`text-xs ${item.up ? 'text-emerald-400' : 'text-red-400'}`}>
                        {item.change}
                      </p>
                    </div>
                  ))}
                    ...
                    {todoData.map((todo) => (
                      <div key={todo.id} className="flex items-start gap-3 p-3 rounded-2xl bg-[#0d0e10]">
                        <button className={`mt-0.5 w-5 h-5 rounded-lg flex items-center justify-center ${
                          ...
                          {todo.completed && <CheckCircle2 className="w-4 h-4 text-[#0d0e10]" />}
                        </button>
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <span className="px-2 py-1 rounded-full bg-emerald-500/20 text-emerald-400 text-xs">多云转晴</span>
                        </div>
                      </div>
                      <Cloud className="w-16 h-16 text-emerald-400/30" />
                    </div>
                  ...
                  {newsData.map((news) => (
                    <div key={news.id} className="relative pl-4 border-l-2 border-emerald-500/30">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {news.tag && (
                          <span className={`text-xs font-bold ${news.tagColor}`}>{news.tag}</span>
                        )}
                      </div>
